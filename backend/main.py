...
async def get_neo4j_graph():
    """Strategy graph nodes and BEATS edges for the 3D visualisation."""
    try:
        from backend.neo4j_client import get_neo4j_client
        client = get_neo4j_client()
        return await client.get_graph_data()
    except Exception as e:
        logger.warning("Failed to get Neo4j graph data: %s", e)
        return {"nodes": [], "links": []}


@app.get("/api/neo4j/win-matrix")
async def get_neo4j_win_matrix():
    """Personality vs personality win/loss matrix."""
    try:
        from backend.neo4j_client import get_neo4j_client
        client = get_neo4j_client()
        ...
        logger.warning("Failed to get Neo4j win matrix: %s", e)
        return {"matrix": []}
...
async def websocket_match(websocket: WebSocket, match_id: str):
    ...
    try:
        ...
        # Keepalive: send a ping every 20 s so proxies/browsers don't time out
        # during long Bedrock inference calls.
        ...
        # Close gracefully so the frontend's ws.onclose fires after match_end
        try:
...
# ---------------------------------------------------------------------------
# CopilotKit LangGraph endpoint (arena commentator)
# ---------------------------------------------------------------------------

try:
    from copilotkit import CopilotKitRemoteEndpoint, LangGraphAgent
    from copilotkit.integrations.fastapi import add_fastapi_endpoint
    from backend.commentator_agent import (
        commentator_graph,
        set_match_store as set_commentator_store,
    )

    ck_agent = LangGraphAgent(
        name="arena-commentator",
        graph=commentator_graph,
    )
    ck_endpoint = CopilotKitRemoteEndpoint(agents=[ck_agent])
    add_fastapi_endpoint(app, ck_endpoint, "/copilotkit_api")
    set_commentator_store(_matches)
    logger.info("CopilotKit LangGraph endpoint mounted at /copilotkit_api")
except Exception as e:
    logger.info("CopilotKit LangGraph endpoint not available: %s", e)

# ---------------------------------------------------------------------------
# AG-UI protocol endpoint — fallback (CopilotKit / arena commentator)
# ---------------------------------------------------------------------------
